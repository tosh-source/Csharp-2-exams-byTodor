...
namespace _14.sep._2013_Task3_Trails3D
{
	class Trails3D
	{
		...
		private static int CalculateDistance(char[,] matrix, int rowStartPoint, int colStartPoint, int row, int col, sbyte playerResult)
		{
			...
			//Calculation
			//calc row distance
			int rowDistance = Math.Abs((row + 1) - (rowStartPoint + 1));
			...
			{
				value = matrix.GetLength(1) - col; 

				if (col > value)
				{
					colDistance = value + colStartPoint;
				}
				else if (col <= value)
				{
					colDistance = col - colStartPoint;
				}
			}
			...
			{
				colDistance = colStartPoint - col;
			}


			int distCounter = rowDistance + colDistance;
